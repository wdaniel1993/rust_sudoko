...
impl From<[[Option<Digit>; 9]; 9]> for Game {
    fn from(definition: [[Option<Digit>; 9]; 9]) -> Self {
        let fields =
            definition.map(|row| row.map(|field| Arc::new(RwLock::new(Field { content: field }))));
        let rows = (0..9).map(|x| FieldGroup {
            group_type: FieldGroupType::Row(x),
            fields: fields[usize::from(x)].to_vec(),
        });
        let columns = (0..9).map(|x| FieldGroup {
        ...
        let shapes = (0..9).map(|x| {
            ...
            let fields = rows
                .into_iter()
                .flat_map(|row| row[column_lower_bound..column_upper_bound].to_vec())
                .collect();
            FieldGroup {
...
mod tests {
    ...
    fn game_creation_with_array_works() {
        ...
        assert_eq!(
            game.groups.get(0).unwrap().group_type,
            FieldGroupType::Row(0)
        );
        assert_eq!(
            game.groups.get(18).unwrap().group_type,
            FieldGroupType::Shape(0)
        );
    }

    #[test]
    fn game_creation_with_positions() {
        let game = Game::try_from(vec![DigitPosition {
